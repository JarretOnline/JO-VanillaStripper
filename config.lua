...
-- Framework
Config.Framework					= 'standalone'		-- Choose which framework your server is using so qb-lapdance can work with it [esx, qbcore, standalone]
...
-- Lap dance 
Config.LapDanceCost					= 200 		-- Cost of the lap dance
...
Config.NudityAge                    = 14        -- (QBCore only!!) Player age restriction. If underage and 'Config.Nudity' is set to true, the stripper won't be topless. Set to 0 if you don't want age restriction

-- Blip
Config.Blip                         = true         -- Set to false if you don't want the blip on the map
Config.BlipName                     = "XXX Strip Club" -- Blip name
Config.BlipStripclub = {
	Sprite = 279,  -- Sprite list can be found here: https://docs.fivem.net/docs/game-references/blips/
	Colour = 61,   -- Color list in the same URL as above
	Display = 6,   -- Explanation here: https://docs.fivem.net/natives/?_0x9029B2F3DA924928
	Scale = 0.6    -- Self explanatory, let you set the scale of the blip
}
Config.BlipCoord = {
	{x=117.81, y=-1298.27, z=29.28}               -- If you want to move the blip on the map, change this.
}
